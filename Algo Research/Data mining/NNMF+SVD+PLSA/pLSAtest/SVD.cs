...
namespace pLSAtest
{
    ...
    static class SVD
    {
        ...
        private static void store(int n, int isw, int j, double[] s)
        {
            switch (isw)
            {
                ...
                case (int)storeVals.RETRQ:
                    if (LanStore[j + MAXLL] == null)
                    {
                        Console.WriteLine("svdLAS2: store (RETRQ) called on index {0} (not allocated) ", (j + MAXLL));
                    }
                ...
                case (int)storeVals.STORP:
                    if (j >= MAXLL)
                    {
                        Console.WriteLine("svdLAS2: store (STORP) called with j >= MAXLL");
                        break;
                    }
                    if (LanStore[j] == null)
                    {
                ...
                case (int)storeVals.RETRP:
                    if (j >= MAXLL)
                    {
                        Console.WriteLine("svdLAS2: store (RETRP) called with j >= MAXLL");
                        break;
                    }
                    if (LanStore[j] == null)
                    {
                        Console.WriteLine("svdLAS2: store (RETRP) called on index %d (not allocated) " + j.ToString());
                    }
        ...
        public static void ProcessData(string datafile, string resultnameblock, bool doTranspose)
        {
            ...
            Console.WriteLine("NUMBER OF ROWS {0}", A.rows);
            Console.WriteLine("NUMBER OF COLS {0}", A.cols);
            if (doTranspose)
            {
                A = svdTransposeS(A);
            }
            Console.WriteLine("Computing the SVD...\n");
            SVDRec R = svdLAS2(A);
            if (R == null)
            {
                Console.WriteLine("Error SVD matrix");
                Environment.Exit(1);
            }
